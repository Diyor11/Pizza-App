...
export default function Promocode({cost}) {
  return (
    <div className='mt-5 bg-white px-5 py-3 flex items-center justify-between rounded-md'>
        <form onSubmit={e => e.preventDefault()} className='flex items-center w-full'>
            <input type="text" className='h-10 rounded-l border border-[f0f0f0] px-3 text-sm w-full sm:w-48' placeholder="Промокод" />
        ...
        <h3 className='font-semibold text-orange-500 tracking-wide hidden sm:flex'>
            Итого:  {cost} ₽
        </h3>
    </div>
  )
